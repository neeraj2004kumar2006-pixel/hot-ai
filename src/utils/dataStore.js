...
import { initialDummyArticles, initialTechTricks, initialAiTools } from './dummyData';
...
const initializeDB = () => {
  ...
  if (!localStorage.getItem(STORAGE_KEYS.TECH) || localStorage.getItem(STORAGE_KEYS.TECH) === '[]') {
    localStorage.setItem(STORAGE_KEYS.TECH, JSON.stringify(initialTechTricks));
  }

  if (!localStorage.getItem(STORAGE_KEYS.TOOLS) || localStorage.getItem(STORAGE_KEYS.TOOLS) === '[]') {
    localStorage.setItem(STORAGE_KEYS.TOOLS, JSON.stringify(initialAiTools));
  }
};
